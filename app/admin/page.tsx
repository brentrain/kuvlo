...
import { ANALYTICS_EXCLUDED_KEY } from "../components/AnalyticsTracker";
...
type Overview = {
  ...
  analytics: { available: boolean; totalViews: number; uniqueVisitors: number; viewsToday: number; viewsWeek: number; viewsMonth: number; uniqueToday: number; uniqueMonth: number; publicSignups: number; signupConversionPercent: number; facebookViews: number; facebookVisitors: number; topPages: { name: string; count: number }[]; devices: { name: string; count: number }[]; referrers: { name: string; count: number }[]; campaigns: { name: string; count: number }[] };
...
export default function AdminPage() {
  ...
  useEffect(() => {
    setDeviceExcluded(window.localStorage.getItem(ANALYTICS_EXCLUDED_KEY) === "true");
    async function load() {
  ...
  return <main className="min-h-screen bg-[#070b18] text-white">
    ...
    <section className="rounded-2xl border border-white/10 bg-slate-900/80 p-5"><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="text-xl font-bold text-white">Website analytics</h2><p className="mt-1 text-sm text-slate-400">Privacy-conscious traffic totals. Creator and admin pages are excluded.</p></div><button onClick={() => { const next = !deviceExcluded; window.localStorage.setItem(ANALYTICS_EXCLUDED_KEY, String(next)); setDeviceExcluded(next); }} className={`rounded-xl border px-4 py-2 text-sm font-bold ${deviceExcluded ? "border-emerald-300/30 bg-emerald-300/10 text-emerald-200" : "border-amber-300/30 bg-amber-300/10 text-amber-200"}`}>{deviceExcluded ? "This device is excluded" : "Exclude this device"}</button></div>{data.analytics.available ? <><div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-5">{[["Views today",data.analytics.viewsToday],["Unique today",data.analytics.uniqueToday],["Facebook views",data.analytics.facebookViews],["Facebook visitors",data.analytics.facebookVisitors],["Public signups",data.analytics.publicSignups],["Views this week",data.analytics.viewsWeek],["Views this month",data.analytics.viewsMonth],["Unique this month",data.analytics.uniqueMonth],["All recorded views",data.analytics.totalViews],["All unique visitors",data.analytics.uniqueVisitors],["Visitor to signup",`${data.analytics.signupConversionPercent}%`]].map(([label,value]) => <div key={String(label)} className="rounded-xl bg-white/5 p-4"><p className="text-xs font-bold uppercase tracking-wide text-slate-400">{label}</p><p className="mt-1 text-2xl font-black text-white">{value}</p></div>)}</div><div className="mt-4 grid gap-4 lg:grid-cols-4">{[["Campaigns",data.analytics.campaigns],["Top pages",data.analytics.topPages],["Devices",data.analytics.devices],["Referrals",data.analytics.referrers]].map(([title,items]) => <div key={String(title)} className="rounded-xl border border-white/10 bg-white/[.03] p-4"><h3 className="font-bold text-white">{String(title)}</h3><div className="mt-3 space-y-2">{(items as {name:string;count:number}[]).length ? (items as {name:string;count:number}[]).map((item) => <div key={item.name} className="flex items-center justify-between gap-3 text-sm"><span className="truncate text-slate-300">{item.name}</span><span className="font-bold text-white">{item.count}</span></div>) : <p className="text-sm text-slate-500">No visits recorded yet.</p>}</div></div>)}</div></> : <p className="mt-4 rounded-xl bg-amber-300/10 p-4 text-sm text-amber-200">Analytics database setup is still required.</p>}</section>
